...
contains
      ...
      if (par%vegetation == 1) then
         ...
         if (xmaster) then
            ...
            do is=1,par%nveg  ! for each species
               ...
               veg(is)%Cd   =      readkey_dblvec(veg(is)%name,'Cd',veg(is)%nsec,size(veg(is)%Cd),  -7.0d0,  0.0d0,      3d0, bcast=.false. )
   ...
   subroutine bulkdragcoeff(s,par,ahh,m,i,j,Cdterm)
      ...
      real*8               :: Qr     ! [-] modified Re
      real*8               :: Re      ! Reynolds number 
      real*8               :: Visc   ! viscosity of water
      real*8               :: alfaw  ! ratio between water depth and plant height 
      integer              :: myflag ! 1 => Ozeren et al. (2013); 2 => Mendez and Losada (2004)
      !
      !
      myflag = s%Cdveg(i,j,m)
      !
     Visc = 0.000001d0
     ...
     elseif (myflag == -3.00) then
       Qr = Re/((alfaw**-1.d0)**5.d0)
       if (Qr >= 200.0) then
         Cdterm = 0.411d0+(514.d0/Qr)**0.5d0
       else
         Cdterm = 0.411d0+(514.d0/200.0)**0.5d0
       endif

      !Cdterm = 0.205d0+(1329.d0/Re) Garzon et al 2018
       
     elseif (myflag == -4.00) then
         Q = KC/((alfaw**-1.d0)**9.d0)
        if (Q >= 125.0) then
            Cdterm = 0.247 + (77.1/Q)**0.25d0
        else
            Cdterm = 0.247 + (77.1/125.0)**0.25d0
        endif
      ! Approach from Anderson and Smith (2014), 
     elseif (myflag == -5.00) then
      
        if (KC >= 40.0) then
            Cdterm = 1.10 + (27.4/KC)**3.08d0
        else
            Cdterm = 1.10 + (27.4/40.0)**3.08d0
        endif
      ! Jadhav 2012
     elseif (myflag == -6.00) then
      
        if (Re >= 1000.0) then
           Cdterm = 0.36 + (2600.0/Re)**1.0d0
        else
           Cdterm = 0.36 + (2600.0/1000.0)**1.0d0
        endif
