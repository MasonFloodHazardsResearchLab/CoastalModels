...
Title:  average_horizontal_eddy_viscosity_in_sea_water_wrt_depth
 
Description: Populates the fields  <nodeID> and <value> (average_horizontal_eddy_viscosity_in_sea_water_wrt_depth)
...
try:
    def makeTable():
        ...
        arcpy.AddField_management(newTB,"Eddy","float")
        ...
        while count == 0:
            line = input.readline()
            data = line.split()
            if str(data[0]) == str("average_horizontal_eddy_viscosity_in_sea_water_wrt_depth"):
                count=+1
